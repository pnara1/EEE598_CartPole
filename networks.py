...
class TD3_Agent:
    ...
    def train(self, step):
        ...
        if len(self.replay_buffer) < START_TRAINING_AFTER:
            return
        #2 - sample buffer
        ...
        with torch.no_grad(): #save computation for unnecessary gradient calcs
            ...
            next_action = (next_action + noise).clamp(-1, 1) #clip to valid action range
            target_Q1 = self.target_critic(next_state, next_action)
            target_Q2 = self.target_critic2(next_state, next_action)
            target_Q = torch.min(target_Q1, target_Q2) #take min of two critics
        ...
        #7b - update critic2 network - standard backprop
...
class DDPG_Agent:
    ...
    def train(self, step):
        ...
        with torch.no_grad(): #save computation for unnecessary gradient calcs
            ...
            # target_Q = torch.clamp(target_Q, min=-50, max=50) #avoid extreme Q-values
            target_Q = torch.clamp(target_Q, -REWARD_SCALE*MAX_STEPS, REWARD_SCALE*MAX_STEPS)
            target_Q = reward + (1 - done) * self.gamma * target_Q #r_t + gamma * Q(s', a')
